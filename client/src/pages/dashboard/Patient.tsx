...
import { Calendar, Clock, User as UserIcon, FileText, CreditCard, Download } from "lucide-react";
...
export function PatientHistory() {
  ...
  const generatePrescriptionPDF = async (prescription: any) => {
    const pdf = new jsPDF();
    ...
    pdf.text(`Doctor: Dr. ${getDoctorName(prescription.doctorId)}`, 20, 40);
    pdf.text(`Date: ${format(parseISO(prescription.createdDate), 'PPP')}`, 20, 50);
    
    // Patient Info
    pdf.text(`Patient: ${user?.name}`, 20, 70);
    pdf.text(`Email: ${user?.email}`, 20, 80);
    
    // Medications
    pdf.setFontSize(14);
    pdf.setTextColor(0, 0, 0);
    pdf.text('Medications:', 20, 110);
    
    pdf.setFontSize(10);
    pdf.setTextColor(60, 60, 60);
    prescription.medications.forEach((med: any, index: number) => {
      const yPos = 120 + (index * 30);
      pdf.text(`${index + 1}. ${med.name}`, 30, yPos);
      pdf.text(`Dosage: ${med.dosage}`, 30, yPos + 8);
      pdf.text(`Frequency: ${med.frequency}`, 30, yPos + 16);
      pdf.text(`Duration: ${med.duration}`, 30, yPos + 24);
    });
    
    // Instructions
    pdf.setFontSize(14);
    pdf.setTextColor(0, 0, 0);
    pdf.text('Instructions:', 20, 120 + (prescription.medications.length * 30) + 20);
    
    pdf.setFontSize(10);
    pdf.setTextColor(60, 60, 60);
    const instructions = prescription.instructions.split(' ').reduce((acc: string[], word: string, i: number) => {
      if (i % 8 === 0) acc.push('\n' + word);
      else acc.push(word);
      return acc;
    }, []).join(' ');
    pdf.text(instructions, 20, 130 + (prescription.medications.length * 30) + 20, { maxWidth: 170 });
    
    // Footer
    pdf.setFontSize(8);
    pdf.setTextColor(100, 100, 100);
    pdf.text('Generated by MediCare+ Health Management System', 105, 280, { align: 'center' });
    
    // Save the PDF
    pdf.save(`prescription_${prescription.id}_${user?.name}.pdf`);
  };
